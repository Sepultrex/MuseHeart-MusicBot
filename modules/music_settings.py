...
class MusicSettings(commands.Cog):
    ...
    async def reset(
        ...
        if not channel or channel.guild.id != inter.guild_id:
            raise GenericError(f"**{bot.user.mention} botta yapılandırılmış şarkı isteği kanalı yok (veya kanal silinmiş).**")
    ...
    async def nodeinfo(self, interaction: disnake.AppCmdInter):
        ...
        if not bot.music.nodes:
            raise GenericError("**Müzik sunucusu yok.**")
        ...
        for page in disnake.utils.as_chunks(available_nodes, 6):
            ...
            em = disnake.Embed(color=color, title="Müzik Sunucuları:")
            ...
            for identifier, node in page:
                ...
                if node.website:
                    txt += f'[`Web sitesi sunucusu`]({node.website})\n'
            ...
            if failed_nodes:
                em.add_field(name="**Başarısız olan sunucular** `❌`",
