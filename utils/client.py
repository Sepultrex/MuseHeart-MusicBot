...
class BotPool:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(f"{data['rest_uri']}/v4/info", timeout=45,
                            ...
                            if r.status == 200:
                                ...
                                print(f"❌ - Lavalink Server [{data['identifier']}] - Yanlış şifre!")
                                return
                            elif r.status != 404:
    ...
    def setup(self):
        ...
        def load_bot(bot_name: str, token: str, guild_id: str = None, load_modules_log: bool = False):
            ...
            @bot.event
            async def on_ready():
                print(f'🟢 - {bot.user} - [{bot.user.id}] Çevrimiçi.')
                ...
                for file in files:
                    ...
                    try:
                        ...
                        if not self.bot_ready and load_modules_log:
                            print(f"🟦 - {bot_name} - {filename}.py Yeniden dolduruldu.")
                        load_status["reloaded"].append(f"{filename}.py")
                    except (commands.ExtensionAlreadyLoaded, commands.ExtensionNotLoaded):
                        try:
                            self.load_extension(module_filename)
                            if not self.bot_ready and load_modules_log:
                                print(f"🟩 - {bot_name} - {filename}.py yüklendi.")
                            load_status["loaded"].append(f"{filename}.py")
                        except Exception as e:
                            print(f"❌- {bot_name} - Modül Yeniden Yüklenmedi/Yüklenemedi: {filename}")
